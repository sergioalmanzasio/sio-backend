...
export const getAllServiceRequests = async (req, res) => {
  try {
    ...
    pool.query(
      ...
      (err, result) => {
        if (err) {
          logger.error(`AdminController.getAllServiceRequests - Error SQL: ${err}`);
          return res.status(500).json({
            process: "info",
            message: "Lo sentimos, no se pudo obtener la información de las solicitudes de servicio, inténtelo más tarde.",
          });
        }
    ...
    logger.error(`AdminController.getAllServiceRequests - Error global: ${error}`);
    return res.status(500).json({
      process: "error",
      message: "Lo sentimos, no se pudo obtener la información de las solicitudes de servicio, inténtelo más tarde.",
    });
