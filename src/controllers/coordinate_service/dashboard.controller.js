  ...
  try {
    ...
    if (!status) {
      return res.status(400).json({
        process: "error",
        message: "Lo sentimos, no fue posible obtener la información, intente nuevamente más tarde.",
      });
    }
    ...
    logger.error('Dashboard.Controller.getServiceRequestCountByState Error al obtener cantidad de solicitudes por estado', err, {
      user: req.user,
...
export const getServiceRequestCountByMonth = async (req, res) => {
  try {
    ...
    logger.error('Dashboard.Controller.getServiceRequestCountByMonth Error al obtener cantidad de solicitudes por mes', err, {
      user: req.user,
    });
    return res.status(500).json({
      process: "error",
